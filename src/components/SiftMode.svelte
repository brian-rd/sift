<script lang="ts">
  import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Check, ChevronLeft, ExternalLink, FolderInput, FolderOpen, Keyboard, RotateCcw, Trash2, X } from '@lucide/svelte';
  ...
  export let onReveal: (file: DownloadFile) => void;
  ...
  function showKeyPress(action: 'keep' | 'trash' | 'undo' | 'fileAway') {
    clearTimeout(pressTimer);
    pressedAction = action;
    pressTimer = setTimeout(() => pressedAction = null, 220);
  }
  ...
  function handleKey(event: KeyboardEvent) {
    ...
    const actions: Record<string, () => void> = {
      [shortcuts.keep]: () => { showKeyPress('keep'); void commit('keep'); },
      [shortcuts.trash]: () => { showKeyPress('trash'); void commit('trash'); },
      [shortcuts.undo]: () => { showKeyPress('undo'); void undo(); },
      [shortcuts.fileAway]: () => { showKeyPress('fileAway'); void fileAway(); }
    };
    ...
    if (/^Digit[1-9]$/.test(event.code)) {
      ...
      if (destination) { event.preventDefault(); showKeyPress('fileAway'); void commit('fileAway', destination); }
      return;
    }
    if ((event.ctrlKey || event.metaKey) && event.code === 'KeyZ') { event.preventDefault(); showKeyPress('undo'); void undo(); }
...
<div class="sift-shell">
  <header class="sift-header">
    ...
    {#if !trashImmediately}<button class="undo-top trash-top" on:click={onViewTrash}><Trash2 size={15} /> View Trash {#if trashCount > 0}<span>{trashCount}</span>{/if}<kbd class="sr-only" aria-hidden="true">{shortcutLabel(shortcuts.undo)}</kbd></button>{/if}
  </header>
      ...
      <section class="preview-card">
        <div class="preview-top">
          ...
          <div class="preview-actions">
            <Tooltip text="Show in File Explorer" placement="bottom"><button on:click={() => onReveal(current)} aria-label={`Show ${current.name} in File Explorer`}><FolderOpen size={16} /></button></Tooltip>
            <Tooltip text="Open in default app" placement="bottom"><button on:click={() => onOpen(current)} aria-label={`Open ${current.name} in its default app`}><ExternalLink size={16} /></button></Tooltip>
          </div>
        </div>
        <div class="preview-content">
          ...
          {:else if current.kind === 'text'}
            <div class="text-preview"><h3 class="sr-only">File content preview</h3><div class="text-preview-meta"><span>{current.extension === 'md' ? 'MARKDOWN' : current.extension.toUpperCase() || 'TEXT'} PREVIEW</span><span>{formatBytes(current.size)}</span></div>{#if textPreview}{#if current.extension === 'md'}<div class="markdown-preview"><MarkdownPreview source={textPreview} /></div>{:else}<pre>{textPreview}</pre>{/if}{:else if textPreviewError}<p>{textPreviewError}</p>{:else}<p>Loading preview…</p>{/if}</div>
          {:else}
  ...
  {#if showDestinations}
    <div class="overlay" role="presentation" on:click={(event) => event.currentTarget === event.target && (showDestinations = false)}>
      <div class="destination-dialog" role="dialog" aria-modal="true" aria-labelledby="destination-title">
        <header><div><p>Quick destination</p><h2 id="destination-title">Where should this file go?</h2></div><Tooltip text="Close destinations" placement="bottom"><button on:click={() => showDestinations = false} aria-label="Close destinations"><X size={18} /></button></Tooltip></header>
...
<style>
  ...
  .trash-top>span{min-width:20px;height:20px;display:grid;place-items:center;padding:0 6px;border-radius:999px;background:var(--danger-bg);color:var(--danger-text);font-size:9px}
  .type-pill{height:22px;display:inline-flex;align-items:center;padding:0 8px;line-height:1}.preview-actions{display:flex;gap:7px}.preview-content{min-width:0;min-height:0;overflow:hidden}.preview-content video{display:block;min-width:0;min-height:0;max-width:100%;max-height:100%;object-fit:contain}.text-preview{width:86%;height:82%;padding:24px 28px}.text-preview-meta{position:sticky;top:-24px;z-index:2;display:flex;justify-content:space-between;margin:-24px -28px 18px;padding:13px 18px;border-bottom:1px solid var(--border);background:var(--surface)}.text-preview-meta span{font-size:8px;letter-spacing:.1em;color:var(--text-3)}.markdown-preview{padding-bottom:12px}.action-dock{gap:10px;padding:12px 18px 16px}.action{height:78px;gap:12px;transition:border-color .16s,background .16s,color .16s,box-shadow .16s,transform .16s}.action strong{font-size:13px}.action small{font-size:9px;margin-top:3px}.action :global(svg){width:21px;height:21px}.action.pressed,.action:active{transform:translateY(1px);border-color:var(--accent);background:var(--surface-3);color:var(--ink);box-shadow:inset 0 0 0 1px var(--accent),0 0 0 3px var(--focus-ring)}.action.pressed>span{background:var(--accent);color:var(--accent-contrast)}.action.file-away{background:var(--primary-action-bg);color:var(--primary-action-text);border-color:var(--primary-action-bg)}.action.file-away:hover,.action.file-away.pressed{background:var(--primary-action-hover);color:var(--primary-action-text);border-color:var(--accent)}.action.file-away:active{background:var(--primary-action-pressed);color:var(--primary-action-text);border-color:var(--accent-pressed)}.action.file-away>span{background:var(--primary-action-key-bg);color:var(--primary-action-text)}.action.file-away small{color:var(--primary-action-muted)}
</style>
