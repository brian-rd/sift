...
<div class="sift-shell">
  <header class="sift-header">
    ...
    <button class="undo-top trash-top" on:click={onViewTrash}><Trash2 size={15} /> View Trash {#if trashCount > 0}<span>{trashCount}</span>{/if}<kbd class="sr-only" aria-hidden="true">{shortcutLabel(shortcuts.undo)}</kbd></button>
  </header>
      ...
      <section class="preview-card">
        <div class="preview-top">
          ...
          <div class="preview-actions">
            <button on:click={() => onReveal(current)} aria-label={`Show ${current.name} in File Explorer`} title="Show in File Explorer"><FolderOpen size={16} /></button>
            <button on:click={() => onOpen(current)} aria-label={`Open ${current.name} in its default app`} title="Open in default app"><ExternalLink size={16} /></button>
          </div>
        </div>
...
<style>
  ...
  .type-pill{height:22px;display:inline-flex;align-items:center;padding:0 8px;line-height:1}.preview-actions{display:flex;gap:7px}.preview-content{min-width:0;min-height:0;overflow:hidden}.preview-content video{display:block;min-width:0;min-height:0;max-width:100%;max-height:100%;object-fit:contain}.text-preview{width:86%;height:82%;padding:24px 28px}.text-preview-meta{position:sticky;top:-24px;z-index:2;display:flex;justify-content:space-between;margin:-24px -28px 18px;padding:13px 18px;border-bottom:1px solid var(--border);background:var(--surface)}.text-preview-meta span{font-size:8px;letter-spacing:.1em;color:var(--text-3)}.markdown-preview{padding-bottom:12px}.action-dock{gap:10px;padding:12px 18px 16px}.action{height:78px;gap:12px;transition:border-color .16s,background .16s,color .16s,box-shadow .16s,transform .16s}.action strong{font-size:13px}.action small{font-size:9px;margin-top:3px}.action :global(svg){width:21px;height:21px}.action.pressed,.action:active{transform:translateY(1px);border-color:var(--accent);background:var(--surface-3);color:var(--ink);box-shadow:inset 0 0 0 1px var(--accent),0 0 0 3px rgba(144,202,249,.18)}.action.pressed>span{background:var(--accent);color:var(--accent-contrast)}.action.file-away{background:var(--primary-action-bg);color:var(--primary-action-text);border-color:var(--primary-action-bg)}.action.file-away:hover,.action.file-away.pressed,.action.file-away:active{background:var(--primary-action-hover);color:var(--primary-action-text);border-color:var(--accent)}.action.file-away>span{background:var(--primary-action-key-bg);color:var(--primary-action-text)}.action.file-away small{color:var(--primary-action-muted)}
</style>
