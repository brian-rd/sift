...
<div class="sift-shell">
  ...
  {:else}
    ...
    <footer class="action-dock" aria-label="Sift actions">
      ...
      <button class="action trash" on:click={() => commit('trash')} disabled={busy}><span>{shortcutLabel(shortcuts.trash)}</span><Trash2 size={18} /><div><strong>Trash</strong><small>Stage for review</small></div></button>
