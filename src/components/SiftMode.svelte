  ...
  function handleKey(event: KeyboardEvent) {
    ...
    if (/^Digit[1-9]$/.test(event.code)) {
      ...
      if (destination) {
        event.preventDefault();
        showKeyPress('fileAway');
        void commit('fileAway', destination);
      }
      return;
    }
...
<div class="sift-shell">
  <header class="sift-header">
    ...
    <div class="progress-meta">
      <div class="progress-copy">
        <strong>{Math.min(processed + 1, total)} of {total}</strong><span>{progress}% complete</span>
      </div>
      <div
        class="progress"
        role="progressbar"
        aria-label="Sifting progress"
  ...
  {:else}
    <main class="sift-main">
      <section class="preview-card">
        <div class="preview-top">
          ...
          <div class="preview-actions">
            <Tooltip text="Show in File Explorer" placement="bottom"
              ><button on:click={() => onReveal(current)} aria-label={`Show ${current.name} in File Explorer`}
                ><FolderOpen size={16} /></button
              ></Tooltip
            >
            <Tooltip text="Open in default app" placement="bottom"
              ><button on:click={() => onOpen(current)} aria-label={`Open ${current.name} in its default app`}
        ...
        <div class="preview-content">
          ...
          {:else if current.kind === 'text'}
            <div class="text-preview">
              <h3 class="sr-only">File content preview</h3>
              <div class="text-preview-meta">
                <span
                  >{current.extension === 'md' ? 'MARKDOWN' : current.extension.toUpperCase() || 'TEXT'} PREVIEW</span
                ><span>{formatBytes(current.size)}</span>
              </div>
              {#if textPreview}{#if current.extension === 'md'}<div class="markdown-preview">
                    <MarkdownPreview source={textPreview} />
                  </div>{:else}<pre>{textPreview}</pre>{/if}{:else if textPreviewError}<p>
                  {textPreviewError}
                </p>{:else}<p>Loading preview…</p>{/if}
            </div>
          {:else}
            <div class="generic-preview">
              <FileIcon kind={current.kind} extension={current.extension} size={54} /><strong
                >{current.extension.toUpperCase() || 'FILE'}</strong
      ...
      <section class="file-details">
        ...
        <dl class="metadata-grid">
          ...
          <div>
            <dt>File type</dt>
            <dd>{current.extension.toUpperCase() || 'Unknown'}</dd>
          </div>
          <div>
            <dt>Date modified</dt>
            <dd>{formatDate(current.modifiedAt)}</dd>
          </div>
          <div>
            <dt>Date created</dt>
        ...
        {#if currentSuggestions.length > 0}
          <div class="suggestion">
            <span class="suggestion-icon"><FolderInput size={19} /></span>
            <div>
              <span
                >{currentSuggestions.length === 1
                  ? 'Suggested destination'
                  : `${currentSuggestions.length} suggested destinations`}</span
              ><strong>{currentSuggestions[0].name}</strong>{#if current.matchedRule}<small
                  >Based on “{current.matchedRule}” rule</small
                >{/if}
            </div>
          </div>
        {:else}
          <div class="suggestion empty">
            <span class="suggestion-icon"><FolderInput size={19} /></span>
            <div><span>No suggestion yet</span><strong>File Away will open the folder picker</strong></div>
          </div>
        {/if}
        ...
        <div class="keyboard-hint">
          <Keyboard size={15} /><span>Use your arrow keys to sift without slowing down</span>
        </div>
      </section>
    </main>
    ...
    <footer class="action-dock" aria-label="Sift actions">
      <button
        class="action keep"
        class:pressed={pressedAction === 'keep'}
        on:click={() => commit('keep')}
        disabled={busy}
        ><span>{shortcutLabel(shortcuts.keep)}</span><ArrowUp size={18} />
        <div><strong>Keep here</strong><small>Leave in Downloads</small></div></button
      >
      <button
        class="action trash"
        class:pressed={pressedAction === 'trash'}
        on:click={() => commit('trash')}
        disabled={busy}
        ><span>{shortcutLabel(shortcuts.trash)}</span><Trash2 size={18} />
        <div>
          <strong>Trash</strong><small>{trashImmediately ? 'Send to Recycle Bin' : 'Stage for review'}</small>
        </div></button
      >
      <button
        class="action undo-action"
        class:pressed={pressedAction === 'undo'}
        on:click={undo}
        disabled={busy}
        ><span>{shortcutLabel(shortcuts.undo)}</span><RotateCcw size={18} />
        <div><strong>Undo</strong><small>Restore the last file</small></div></button
      >
      <button
        class="action file-away"
        class:pressed={pressedAction === 'fileAway'}
        on:click={fileAway}
        disabled={busy}
        ><span>{shortcutLabel(shortcuts.fileAway)}</span><FolderInput size={18} />
        <div>
          <strong>File Away</strong><small
            >{currentSuggestions.length === 1
              ? currentSuggestions[0].name
              : currentSuggestions.length > 1
                ? 'Choose a suggestion'
                : 'Choose a folder'}</small
          >
        </div></button
      >
    </footer>
  ...
  {#if showDestinations}
    <div
      class="overlay"
      role="presentation"
      on:click={(event) => event.currentTarget === event.target && (showDestinations = false)}
    >
      <div class="destination-dialog" role="dialog" aria-modal="true" aria-labelledby="destination-title">
        <header>
          <div>
            <p>Quick destination</p>
            <h2 id="destination-title">Where should this file go?</h2>
          </div>
          <Tooltip text="Close destinations" placement="bottom"
            ><button on:click={() => (showDestinations = false)} aria-label="Close destinations"
              ><X size={18} /></button
            ></Tooltip
          >
        </header>
        <div class="destination-list">
          {#each destinationOptions as destination, index}<button
              on:click={() => commit('fileAway', destination)}
              ><span>{index + 1}</span><FolderInput size={18} />
              <div><strong>{destination.name}</strong><small>{destination.path}</small></div>
        ...
        <button
          class="choose-other"
          on:click={async () => {
            const destination = current && (await onPickDestination(current));
            if (destination) await commit('fileAway', destination);
...
<style>
  ...
  .sift-main {
    flex: 1;
    display: grid;
    grid-template-columns: minmax(360px, 1.22fr) minmax(340px, 0.78fr);
    gap: 46px;
    align-items: center;
    max-width: 1120px;
    width: 100%;
  ...
  .generic-preview :global(.file-icon) {
    width: 92px;
    height: 92px;
    border-radius: 18px;
  }
  ...
  .file-details h1,
  .complete-card h1 {
    font: 650 28px/1.18 var(--font-display);
  ...
  .action.file-away > span {
    background: rgba(255, 255, 255, 0.13);
    color: #fff;
  }
  .action.file-away small {
  ...
  .destination-dialog header button {
    width: 34px;
    height: 34px;
    border: 0;
    border-radius: 8px;
  ...
  .action.file-away:hover,
  .action.file-away.pressed {
    background: var(--primary-action-hover);
    color: var(--primary-action-text);
    border-color: var(--accent);
  }
  .action.file-away:active {
