...
<section class="rules-card">
  ...
  {#each rules as rule, index}
    <article class:disabled={!rule.enabled}>
      <div class="priority"><GripVertical size={15} /><strong>{index + 1}</strong><div><button on:click={() => moveRule(index,-1)} disabled={index === 0} aria-label={`Move ${rule.name} up`}><ArrowUp size={11} /></button><button on:click={() => moveRule(index,1)} disabled={index === rules.length - 1} aria-label={`Move ${rule.name} down`}><ArrowDown size={11} /></button></div></div>
      <div class="rule-name"><strong>{rule.name}</strong><span>{rule.matches} {rule.matches === 1 ? 'match' : 'matches'}</span></div>
      ...
      <button class="toggle" on:click={() => toggleRule(rule.id)} aria-label={`${rule.enabled ? 'Disable' : 'Enable'} ${rule.name}`} aria-pressed={rule.enabled}>{#if rule.enabled}<ToggleRight size={28} />{:else}<ToggleLeft size={28} />{/if}</button>
      <button class="more" aria-label={`More options for ${rule.name}`}><MoreHorizontal size={17} /></button>
    </article>
  {/each}
</section>
...
{#if showEditor}
  <div class="modal-backdrop" role="presentation" on:click={(e) => e.currentTarget === e.target && (showEditor = false)}>
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="new-rule-title">
      <header><div><p class="eyebrow">Rule builder</p><h2 id="new-rule-title">Create a sorting rule</h2></div><button on:click={() => showEditor = false} aria-label="Close"><X size={18} /></button></header>
...
{#if showPreview}
  <div class="modal-backdrop" role="presentation" on:click={(e) => e.currentTarget === e.target && (showPreview = false)}>
    <div class="modal preview-modal" role="dialog" aria-modal="true" aria-labelledby="preview-title">
      <header><div><p class="eyebrow">Dry run</p><h2 id="preview-title">Preview {matchedFiles.length} changes</h2></div><button on:click={() => showPreview = false} aria-label="Close"><X size={18} /></button></header>
