<script lang="ts">
  ...
  $: matched = ruleMatches.map(({ file, rule }) => ({ ...file, matchedRule: rule.name, suggestedFolder: rule.destination }));
  $: largeOrOld = files.filter((file) => file.size > 75_000_000 || Date.now() - file.modifiedAt > 30 * 86_400_000);
  $: recent = files
    .map((file) => matchByPath.has(file.path) ? { ...file, matchedRule: matchByPath.get(file.path)?.name, suggestedFolder: matchByPath.get(file.path)?.destination } : file)
...
<div class="dashboard-grid">
    ...
    <div class="file-list">
      {#each recent as file}
        <div class="file-row">
          ...
          <div class="file-name"><Tooltip text={file.name}><strong>{file.name}</strong></Tooltip><span>{formatBytes(file.size)} · {timeAgo(file.modifiedAt)}</span></div>
  ...
  <section class="panel rules-panel">
    <div class="section-heading"><div><p class="eyebrow">Automation</p><h2>Rules ready</h2></div><Tooltip text="Open rules"><button class="icon-button" on:click={onRules} aria-label="Open rules"><ListChecks size={17} /></button></Tooltip></div>
...
<style>
  ...
  .primary{color:var(--accent-contrast)}.primary:active{background:var(--accent-pressed)}.stats article,.panel{background:var(--surface)}.stat-icon.success,.matched{background:var(--success-bg);color:var(--success-text)}.ring{border-color:var(--success-bg);border-top-color:var(--accent-pressed)}.secondary{background:var(--surface)}.secondary:hover{background:var(--surface-2)}
</style>
