<script lang="ts">
  ...
  function toggle(operationId: number) {
    selected = selected.includes(operationId)
  ...
  <div class="trash-dialog" role="dialog" aria-modal="true" aria-labelledby="trash-title">
    <header class="dialog-header">
      <div class="title-block">
        <span class="title-icon"><Trash2 size={20} /></span>
        <div>
          <p>{intent === 'review' ? 'Sift Trash' : 'Before you go'}</p>
          <h2 id="trash-title">Review Trash</h2>
          <span>{items.length} {items.length === 1 ? 'file is' : 'files are'} waiting for your decision.</span
          >
        </div>
      </div>
      <Tooltip text="Close Trash" placement="bottom"
        ><button class="icon-button" on:click={onClose} aria-label="Close Trash"><X size={18} /></button
        ></Tooltip
      >
    </header>
    ...
    {#if items.length > 0}
      <div class="trash-toolbar">
        <label
          ><input type="checkbox" checked={allSelected} on:change={toggleAll} /><span>Select all</span></label
        >
      ...
      <div class="trash-list">
        {#each items as item (item.operationId)}
          <article
            ...
            on:contextmenu={(event) => showContextMenu(event, item)}
          >
            <label class="select-file" aria-label={`Select ${item.name}`}
              ><input
                type="checkbox"
                checked={selected.includes(item.operationId)}
                on:change={() => toggle(item.operationId)}
              /></label
            >
      ...
      <div class="selection-actions">
        ...
        >
        <button
          class="restore-selected"
          on:click={() => onRestore(selected)}
          disabled={busy || selected.length === 0}><RotateCcw size={15} /> Restore selected</button
        >
        <button
          class="recycle-selected"
          on:click={() => onRecycle(selected)}
          disabled={busy || selected.length === 0}><Trash2 size={15} /> Move to Recycle Bin</button
        >
      </div>
    {:else}
      <div class="empty-trash">
        <span><CheckSquare size={30} /></span>
        <h3>Trash is empty</h3>
        <p>Every staged file has been restored or moved to the Windows Recycle Bin.</p>
      </div>
    {/if}
    ...
    <footer>
      ...
      {:else}
        <button class="secondary" on:click={onClose}
          >{intent === 'exit' ? 'Keep Sift open' : 'Keep sifting'}</button
        >
        <button class="continue" on:click={onContinue}
          >{intent === 'exit' ? 'Exit Sift' : 'Continue to overview'}</button
        >
      {/if}
    </footer>
  </div>
  ...
  {#if context}
    <div class="context-menu" style={`left:${context.x}px;top:${context.y}px`} role="menu" tabindex="-1">
      <button
        role="menuitem"
        on:click={() => {
          onRestore([context!.item.operationId]);
          context = null;
        }}><RotateCcw size={14} /> Undo trash</button
      >
      <button
        role="menuitem"
        class="danger"
        on:click={() => {
          onRecycle([context!.item.operationId]);
          context = null;
        }}><Trash2 size={14} /> Move to Recycle Bin</button
      >
    </div>
  {/if}
</div>
...
<style>
  ...
  .trash-list article {
    min-height: 70px;
    display: grid;
    grid-template-columns: 22px 42px minmax(0, 1fr) 84px;
    gap: 11px;
    align-items: center;
    padding: 9px 20px;
    border-bottom: 1px solid var(--border);
  }
  .trash-list article:hover {
    background: var(--surface-2);
  }
  .select-file {
    display: grid;
